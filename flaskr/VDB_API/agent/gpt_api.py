...
def update_new_processing_message(response, processing_message):
    action_text_dict = {
    ...
    if action is not None and action in action_text_dict.keys() and query is not None:
        ...
        processing_message.edit(content=return_message + query)
        return

    if action is not None and action in action_text_dict.keys():
        return_message = action_text_dict[action]
        processing_message.edit(content=return_message)
        return


def llm_agent(query, model, selected_tools, processing_message=None):
    docs = []
    ...
    if isinstance(model, HuggingFacePipeline):
        ...
        response = model.invoke('你今天好嗎?', stop=stop)
    else:
        response = model.invoke(prompt, stop=stop).content
    print('response :', response)
    if processing_message is not None:
        update_new_processing_message(response, processing_message)
    for _ in range(3):
        ...
        if type(api_output) == tuple:
            api_output = str(api_output[0])  # 將 api 輸出轉為字串
        else:
